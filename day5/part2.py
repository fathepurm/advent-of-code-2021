...
def update_xy_diagonal(line: Line) -> None:
    x_min: int = min(line.start_x, line.end_x)
    x_max: int = max(line.start_x, line.end_x)

    if abs(line.start_x - line.end_x) == abs(line.start_y - line.end_y):
        if line.start_x == x_min:
            y_coordinate = line.start_y
            what_to_add = -1 if y_coordinate > line.end_y else 1
        else:
            y_coordinate = line.end_y
            what_to_add = -1 if y_coordinate > line.start_y else 1
        while x_min <= x_max:
            matrix[x_min][y_coordinate] += 1
            x_min += 1
            y_coordinate += what_to_add
